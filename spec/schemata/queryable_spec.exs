...
defmodule Schemata.Queryable.AlterTableSpec do
  ...
  before do
    {:shared, queries: [
      ...
      {"ALTER TABLE users ALTER contacts TYPE list<text>",
       %AlterTable{named: "users", column: :contacts,
                   op: {:alter, {:list, :text}}}},
      {"ALTER TABLE users ADD email text",
       %AlterTable{named: "users", column: :email, op: {:add, :text}}},
      {"ALTER TABLE users ADD friends set<text>",
       %AlterTable{named: "users", column: :friends,
                   op: {:add, {:set, :text}}}},
